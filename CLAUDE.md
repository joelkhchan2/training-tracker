...
- **Prefs** (`features/settings/prefs.ts` + `usePrefs.ts`): a Zustand store persisted to `localStorage['tt-prefs']` (theme, font family/scale, weightUnit, weekStartDay, rest-timer default/haptics, showRpe). `persistApply` spreads the whole current state before applying a patch (a deliberate fix so a newly-added `Prefs` field is never silently dropped by a hand-enumerated setter). The store itself is framework/auth-agnostic; all Supabase I/O lives in `usePrefsSync.ts`. Prefs now sync cross-device via `usePrefsSync` (mounted once, for the app's lifetime, in `App.tsx`'s `PrefsSyncMount`): gated on `onboarding_complete` and keyed on `userId` (a same-id session swap like `TOKEN_REFRESHED` is a no-op). Hydrate is server-wins when `profiles.ui_prefs` is non-null; otherwise it seeds the server from local prefs, reconciling `profiles.units` into `weightUnit` via `unitsToWeightUnit`. Local changes write through to the server debounced (500ms), with one retry (~1s) on failure before logging and giving up. Sign-out or switching to a different user resets the store to `DEFAULT_PREFS` and repaints the DOM (`initPrefs()`) without writing to the server. See the accepted conflict-model gaps under Known issues below.
...
- **`profiles.units` is seed-only:** written once at onboarding (stored as `'lbs'`/`'kg'`, vs. `Prefs.weightUnit`'s `'lb'`/`'kg'`), then superseded by `ui_prefs.weightUnit` once `usePrefsSync`'s seed-up reconciles it via `unitsToWeightUnit`. Not read anywhere after that first seed.
- **Prefs sync conflict model (accepted, not a bug):** for this single-user personal tracker, two open tabs on the same device are last-writer-wins with no cross-tab propagation — a change in one tab isn't reflected in the other until it reloads and re-hydrates. Similarly, an offline write-through that still fails after its one retry can be silently overwritten by the next load's server-wins hydrate. Both are accepted trade-offs, not defects to fix.
...
- **Canonicalization Phase B backup tables — drop after 2026-08-30:** the 2026-07-30 Phase B sweep left prod backup tables `canon_bak_exercises_20260730`, `canon_bak_strength_sets_20260730`, `canon_bak_program_exercises_20260730`, `canon_bak_personal_records_20260730`, `canon_bak_exercise_progress_20260730` in place for rollback (rollback = truncate the target table + `insert into <target> select * from <canon_bak_...>`). Drop them once the sweep is confirmed good, target ~30 days out (2026-08-30).
