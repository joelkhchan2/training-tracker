-- Add duration_seconds to strength_sets for timed/weighted_time exercise logging
-- (front lever holds, dead hangs, planks, weighted hangs). Purely additive: nullable,
-- no default, no check constraint. Existing rows read duration_seconds = null, which
-- inferInputType() (src/domain/duration.ts) already reads as 'weighted' or 'bodyweight'
-- exactly as they display today — nothing is backfilled or reclassified.
--
-- log_workout is replaced (not dropped-and-recreated): duration_seconds is a new key
-- *inside* the existing p_sets jsonb array elements, not a new positional parameter, so
-- `create or replace function` with the same 6-param signature is sufficient — unlike
-- 0005/0006, which each widened the positional argument list and required
-- `drop function if exists` first. The existing grant on this signature (from 0006)
-- still applies; no new grant is needed.
alter table strength_sets add column duration_seconds integer;
