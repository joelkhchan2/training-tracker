 ...
 *  climbing_sends + max_v_grade PR upsert), then invalidates `['sessionHistory']`. Returns the
 *  session id plus PR info: `newMaxGrade` is non-null only when this save set a new max-grade PR
 *  (the caller's signal to celebrate); `previousMaxGrade` is the grade being beaten (or null). */
export function useLogClimbing() {
  const queryClient = useQueryClient()
  return useMutation<LogClimbingResult, Error, LogClimbingInput>({
    mutationFn: async ({ clientId, date, notes, sends }) => {
      const { data, error } = await getSupabase().rpc('log_climbing', {
